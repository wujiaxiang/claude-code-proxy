...
- **gateways/usage_store.py**：用量统计 SQLite 持久化（纯 `sqlite3`）：内存统计异步落盘（`.cache/usage.db`，`init_db()` 建**三张独立表**——`usage_daily`(date,label,model，底层端口直连) / `anthropic_daily`(date，8081 翻译入口独立统计) / `aggregator_daily`(date,vm_id,member，8080 聚合网关独立统计)；三者刻意隔离，避免 8081/8080 转发到下游端口时双计）；公开 `init_db()`/`upsert_day()`/`get_trend()` + `upsert_anthropic_day()`/`get_anthropic_trend()` + `upsert_aggregator_day()`(error_types JSON 合并)/`get_aggregator_trend()`；8081 走 server.py `_ANTHROPIC_ACCUM` + `_flush_anthropic_accum`，8080 走 engine.py `note_request` 旁路 → `_bump_aggregator_usage` → `_AGGREGATOR_ACCUM` + `_flush_aggregator_accum`（均 60s `_usage_flush_loop` UPSERT + 退出前 flush）；dashboard「近 7/30 天请求量」「8081 翻译入口」「8080 聚合网关」三张趋势卡读取；DB 故障仅 warning 不影响主链路
