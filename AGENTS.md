...
- **`targets.json`**（Target 定义，核心配置）：必填 `label / listenPort / category / handler / targetHost`；category ∈ `crack|free|paid|aggregate`；handler ∈ `passthrough|copilot|qclaw|gemini-native|trae-work|aggregator|anthropic`；crack 类必须有 `crackTool`；label/端口不能冲突；`enabled=false` 跳过必填校验（预留位）。secrets 优先级：`secrets.json > apikeyEnv 环境变量 > 客户端透传`。**热重载**：mtime 轮询 2s，改完即生效（含端口动态增删）。可选行为开关：`cleanCodebuddyBody` / `cleanQclawBody` / `normalizeSse`（SSE 帧规范化，修不合规上游）/ `normalizeFinishReason` / `stripV1`（上游 OpenAI 兼容但无 `/v1` 前缀时，把客户端 `/v1/xxx` 剥成 `/xxx`，如 DeepSeek `api.deepseek.com`，配合 `handler=passthrough` 使用）。完整 schema 见 [docs/architecture.md](docs/architecture.md)
...
- **gateways/usage_store.py**：用量统计 SQLite 持久化（纯 `sqlite3`）：内存统计异步落盘（`.cache/usage.db`，`init_db()` 建**三张独立表**——`usage_daily`(date,label,model，底层端口直连) / `anthropic_daily`(date，8081 翻译入口独立统计) / `aggregator_daily`(date,vm_id,member，8080 聚合网关独立统计)；三者刻意隔离，避免 8081/8080 转发到下游端口时双计）；`usage_daily` 新增 `error_types` 列（错误分类直方图，`_classify_http_error`/`_bump_model_error` 在 6 处错误路径采集，label="anthropic" 跳过不混写，`upsert_day` 按 dict 合并）；公开 `init_db()`/`upsert_day()`/`get_trend()` + `upsert_anthropic_day()`/`get_anthropic_trend()` + `upsert_aggregator_day()`(error_types JSON 合并)/`get_aggregator_trend()`；8081 走 server.py `_ANTHROPIC_ACCUM` + `_flush_anthropic_accum`，8080 走 engine.py `note_request` 旁路 → `_bump_aggregator_usage` → `_AGGREGATOR_ACCUM` + `_flush_aggregator_accum`（均 60s `_usage_flush_loop` UPSERT + 退出前 flush）；dashboard「近 7/30 天请求量」「8081 翻译入口」「8080 聚合网关」三张趋势卡读取；DB 故障仅 warning 不影响主链路
